 ...
 *
 * Juga jalur SINKRONISASI antrean offline PWA: klien mengirim `client_uid`
 * (UUID per-penjualan) agar retry setelah jaringan berkedip bersifat idempoten.
 */
class TransaksiController extends Controller
{
    public function store(Request $request, KasirService $service): JsonResponse
    {
        $validated = $request->validate([
            ...
            // Idempotensi sync offline: opsional (penjualan web biasa tidak mengirim).
        ...
        // KasirService::jual idempoten terhadap client_uid: penjualan yang sudah
        // tercatat dikembalikan apa adanya (wasRecentlyCreated=false → 200).
        $transaksi = $service->jual($validated, $request->user()->id);

        return $this->responseTransaksi(
            $transaksi,
            $transaksi->wasRecentlyCreated ? 201 : 200,
        );
    }

    private function responseTransaksi(Transaksi $transaksi, int $status): JsonResponse
    {
